...
const Orders = () => {
    ...
    const handleRemoveCart = (product) => {
        const rest = carts.filter(cart => cart._id !== product._id);
        setCart(rest)
        ...
        <div className='flex md:flex-row flex-col justify-around md:items-start items-center my-10'>
            ...
            <div className='md:sticky top-[92px] md:m-0 m-auto order-1 md:order-2'>
                <CartSummary carts={carts} Clear={Clear} className='bg-slate-700 text-white p-4 rounded-md'>
                    <button onClick={reviowOrder} className='flex justify-center w-full bg-yellow-600 py-3 shadow-lg rounded-lg my-4 hover:bg-yellow-400'>Proceed shipping <ArrowCircleRightIcon className='w-6 ml-3'></ArrowCircleRightIcon></button>
                </CartSummary>
            </div>
        </div>
    );
